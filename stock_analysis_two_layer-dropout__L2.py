...
def create_lstm_model(seq_length, n_features):
    """创建LSTM模型"""
    model = Sequential([
        LSTM(128, return_sequences=True, input_shape=(seq_length, n_features), kernel_regularizer=l2(0.01)),
        # 添加 L2 正则化
        Dropout(0.5),  # 添加 Dropout 层
        LSTM(64, return_sequences=False, kernel_regularizer=l2(0.01)),  # 添加 L2 正则化
        Dropout(0.5),  # 添加 Dropout 层
        Dense(32, kernel_regularizer=l2(0.01)),  # 添加 L2 正则化
        Dense(1)  # 输出层
    ])
